...
- 后续需求、缺陷修复和服务端升级必须考虑用户仍在使用已发布的旧版 App，不得默认客户端会同步升级。
- 修改 API 字段、枚举、错误码、状态机、心跳、催办或播放上报前，检查旧版请求和响应解析方式，明确旧版 App 在新服务端上的行为。
- 优先采用向后兼容的增量变更：新增请求字段提供兼容默认值，新增响应字段允许旧版忽略；不得直接删除、重命名既有字段，改变类型或让原有可选字段变为必填。
- 新增状态或错误码必须评估旧版处理路径，尤其不能造成全屏弹框无法关闭、登录循环、学习操作被阻断或离线队列无法重放。必要时由服务端兼容处理，保持真实业务状态，不伪造成功记录。
- 无法向后兼容时，必须先说明受影响版本、升级要求、发布顺序和回退方案，经用户批准后再实施；不得静默要求所有用户升级。
- 涉及兼容性的变更必须在 Spec / Plan / ADR 中记录兼容策略，并通过对应窄测试或真实请求验证旧版调用；完成汇报明确是否需要升级 App。
