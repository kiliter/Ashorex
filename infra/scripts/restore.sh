...
ATTACHMENTS_TEMP="${DATA_DIR}/.attachments-restore-${STAMP}"
...
attachment_rows=0
if [[ "$(sqlite3 "${BACKUP_FILE}" "select count(*) from sqlite_master where type='table' and name='todo_attachments';")" == "1" ]]; then
  attachment_rows="$(sqlite3 "${BACKUP_FILE}" "select count(*) from todo_attachments;")"
fi
if [[ -f "${ATTACHMENTS_BACKUP}" && -f "${ATTACHMENTS_CHECKSUM}" ]]; then
  ...
  # 备份由本服务生成，仍限制归档内所有路径只能位于 attachments 目录。
  while IFS= read -r entry; do
    if [[ "${entry}" != "attachments" && "${entry}" != "attachments/"* ]]; then
      echo "恢复失败：附件归档包含越界路径。" >&2
      exit 1
    fi
  done < <(tar -tzf "${ATTACHMENTS_BACKUP}")
elif ((attachment_rows > 0)); then
  echo "恢复失败：数据库备份包含Todo 附件记录，但缺少附件归档或校验文件。" >&2
  exit 1
fi
...
if [[ -d "${ATTACHMENTS_DIR}" ]]; then
  mv -- "${ATTACHMENTS_DIR}" "${ARCHIVE_DIR}/attachments"
fi
...
if [[ -f "${ATTACHMENTS_BACKUP}" ]]; then
  ...
  mv -- "${ATTACHMENTS_TEMP}/attachments" "${ATTACHMENTS_DIR}"
