...
- 观看 Emby 视频。
- 允许通过快进按钮每次跳过 10 秒并由服务端认可为通过；跳过不增加实际学习时长，进度条只读（ADR-0024）。
- 记录可信观看时间与可信最大进度。
- 视频中随机验活。
- 视频完成后答题。
- 首页右滑小工具中的番茄钟和练习计时。
...
- 管理员登录。
- 创建和禁用用户。
- 删除普通用户及其全部学习记录，管理员账号受保护，删除前需输入用户名二次确认（ADR-0023）。
- 查看 Emby 连接状态。
...
用户可以自行删除考试目标（ADR-0022）：

- 删除只清除考试目标本身和它的课程绑定。可信观看进度、作战单、学习欠债、答题记录和报表都不属于考试目标，删除后全部保留。
- 服务端校验归属；目标不存在或不属于当前用户返回稳定错误码 `EXAM_GOAL_NOT_FOUND`。
- 允许删除最后一个考试目标，此时首页回到考试目标设置引导。
- 客户端删除前必须二次确认，并明确说明学习记录不会被删除。

首次登录没有考试目标时，首页自动引导一次考试目标设置。设置页保存成功后以 pop 返回首页，首页据此重新读取考试列表。若引导返回后仍没有考试目标，首页只展示空态和显式设置入口，不再自动跳转。
...
- 用户只能通过服务端授权的快进按钮每次跳过 10 秒，跳过视为已通过并推进 `maxVerifiedPosition`，但不增加实际学习时长（ADR-0024）。普通心跳仍拒绝未经授权的位置跳跃。
- 用户可通过快退/快进按钮回看 `0..maxVerifiedPosition`；普通学习和复习进度条均不接受点击、拖拽或无障碍调节定位。
- App 进入后台时自动暂停。
...
- `REVIEW_SHORTCUT` 只允许关联已经可信完成的课时，因此整个视频已处于可回看区间，可用快退/快进按钮回看，进度条只读。
...
1. 后台可以物理删除普通用户及其全部关联记录，删除不可恢复。
2. 管理员账号不可删除，避免后台失去唯一入口。
3. 删除前必须手工输入与目标用户完全一致的用户名，页面另有一次确认；确认不匹配不得执行。
4. 删除必须先显式清除该用户的欠债还债记录，再删除用户行，由 `users` 的级联外键清除其余用户数据。外键约束保留为安全网，漏删被 RESTRICT 引用的表时事务必须失败，不得静默留下孤儿数据。
5. 模拟考试试卷附件的磁盘路径必须在数据库删除之前读取；文件清理在事务提交成功后执行，只允许删除附件受控目录内的文件，清理失败只记录日志，不回滚已完成的删除。
6. 删除只作用于该用户，不删除课程、课时、题库、全文摘要、内容任务等共享数据。
7. 删除会级联清除该用户的 Refresh Token；已签发的 Access Token 在有效期内仍可通过校验，窗口期内的写操作由外键约束阻断。
8. 删除操作记录日志时只包含用户 ID 和附件数量，不记录用户名、显示名或文件路径。
...
- 快进按钮每次前进 10 秒；学习模式先请求服务端授权再跳转，复习模式在全片范围内跳转。进度条只展示当前播放位置与通过位置，不接受定位操作。
- 前台播放期间保持屏幕常亮，暂停、验活、完成、切后台和退出时释放；全屏切换复用相同生命周期。
...
7. 用户播放视频。
8. 按钮快进可跳过未看内容并推进通过位置，但不增加实际学习时长；进度条不能拖动，未授权心跳跳跃仍被拒绝。
9. 触发并通过随机验活。
10. 视频完成。
11. 用户提交课后题。
12. 计划任务完成。
...
3. 已学课时显示为复习快捷入口，可用按钮回看全片，进度条只读且不改变作战单完成率。
...
- 未看区间仅能通过固定 10 秒按钮快进 API 认可为通过，不增加实际学习时长；普通心跳不可冒充快进授权。
