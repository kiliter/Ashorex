...
- 选择器中未开始项目再次点击即取消；已经开始的不可变项目不能取消。
- 视频学习和复习快捷入口严格按课程及课时固有顺序排列，不提供手动调序；模拟考试排列在课时之后。
...
`PUT` 接收 `expectedVersion` 和完整项目数组，在一个事务中保存；版本不一致返回 `PLAN_VERSION_CONFLICT`。只能替换未开始项目，服务端保留不可修改项目并校验客户端没有删除或篡改它们。
...
5. 删除已归档课程只写入 UTC `removed_at` 并从管理界面永久移除，不物理删除课程、课时或学习历史；该课程不能再从后台手工恢复。
6. 单个与批量删除均必须二次确认，展示课程数量、课时总数和“不能手工恢复但学习历史保留”的说明；批量校验或更新失败时整批不写入。
7. 重新添加与已移除课程相同的 Emby 来源时，恢复原课程和课时身份、清除 `removed_at` 并继续同步，结果明确标记为恢复历史课程。
...
5. 重复活动来源不会创建第二门课程；已归档的同来源课程恢复原课程身份。
6. 管理员删除课程时看到保留历史提示，确认后课程从 App 和活动后台列表隐藏，但课时及学习数据仍存在。
7. 管理员从已归档列表恢复课程，原课程 ID、课时 ID、可信进度、计划、欠债、题目和学习内容保持不变。
8. 管理员在已归档课程区域单个删除或勾选多门课程批量删除，二次确认展示课程数和课时数；确认后课程不再出现在归档区域，学习历史仍保留。
