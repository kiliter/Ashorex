...
| POST | `/diagnostics/logs` | 用户手动上传本机诊断日志（multipart，见 ADR-0052） |
...
```text
todo_attachments(+目录) → diagnostic_log_uploads(+目录) → todo_progress_events → todos → todo_deletions
...
已批准扩展运维范围：新增独立 updater（host 网络、无公网端口），在后台「版本与升级」配置自动升级及手动检查/预下载/升级。正式版本 digest 发布，升级前停写备份，健康与版本验收通过才开放业务，失败恢复及中断接续遵循 [ADR-0048](../adr/0048-container-server-upgrade.md)。不改变 App 契约，维护期间旧版收到可重试 503；首次接入需手动安装维护协议与共享卷。不包括客户端在线更新。
...
### 2026-09-15 播放异常结束防误完成

按用户批准的 ADR-0039 修正，异常 completed 不得补满进度；真实位置距已知片尾不超过两秒且无未恢复错误时才接受结束，上报层独立复核，总尾差校正不超过两秒。保留自由拖动与服务端裁决，旧 App 需升级获得修复，不自动纠正历史数据。

### 2026-09-15 视频流持续传输补充

按 ADR-0039，播放代理使用请求虚拟线程同步分块转发，避免默认 Servlet 异步总超时截断视频；固定 64KiB 内存缓冲、失败关闭上游，并记录脱敏传输计数。保留现有接口、Range 与服务端进度判定，旧 App 无需为本项服务端修复升级。
