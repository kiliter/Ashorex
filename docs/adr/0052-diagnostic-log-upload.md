...
## 2026-09-15：MediaKit 内核诊断补充

- 为定位中途异常结束，MediaKit 开启 debug 内核日志，接入现有手动上报文件；不改变完成、进度和观看时长规则。
- 每个适配器记录独立会话编号、单调经过时间、内核位置、总时长、播放、缓冲、结束与错误状态；completed 在状态修改前和位置对齐后分别记录，被片尾状态忽略的错误仍留证据。
- 记录打开、播放、暂停、跳转、倍速命令及耗时、失败；位置每五秒采样，不逐帧落盘。非警告错误的内核消息每秒最多 40 条，下一窗口或销毁时记录省略数量，警告与错误保留。
- 原生文本含认证、地址、请求头、Range 或路径的行整体隐藏，其余文本截断并通过现有脱敏器二次处理；页面仍只显示安全错误文案。不保存媒体地址与请求头。
- 沿用 2 MiB 单文件与一份备份限制。安装新构建、复现异常后尽快从关于页上报日志；旧 App 不会自动获得新增日志。无 API、数据库或旧版兼容变更。
