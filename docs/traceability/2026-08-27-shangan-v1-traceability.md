...
| ADMIN-002 | Web 后台配置 Emby、LLM、ASR 和 MCP 并立即生效 | 18 | `RuntimeIntegrationSettingsServiceTest`、`IntegrationSettingsAdminTest`、外部适配器刷新测试 | 保存后不重启，新调用使用最新配置 |
...
| SEC-003 | 后台配置密钥仅在 ADMIN 页面和服务端存储中可见 | 18 | ADMIN/CSRF/禁止缓存/响应泄露测试 | Flutter、业务 API、日志和错误响应无密钥 |
