...
依据已接受 ADR-0048。文件：infra/updater、Compose/Dockerfile/部署脚本、服务端 upgrade 与后台 Controller、管理后台升级页、CI、运行手册和原型。接口仅 /admin/api/upgrades；维护探测 /internal/upgrade-readiness，不进入 App OpenAPI 范围。任务顺序：窄失败测试 → 独立升级状态机及 Docker 适配 → 管理接口及维护边界 → 页面/原型 → 发布流水线 → 窄测试、真实启动与 CI Docker Smoke → PR 全检查通过后合并。提交主题：feat: 支持 host 网络独立容器自动升级服务端。

### 2026-09-10 T48 页面高度修正

版本与升级页按 ADR-0048 限制在后台可用视口内，标题固定，宽屏卡片分列、窄屏单列；空间不足及长更新说明仅内容区滚动，确认操作始终可达。验证后台类型检查与构建，检查宽屏、窄屏和矮窗口布局。
...
遵循 [ADR-0052](../adr/0052-diagnostic-log-upload.md)。文件：V012、server/diagnostics、Archive 用户级联、Flutter 本机滚动日志与关于页、管理后台查看与删除页、OpenAPI 与原型 6-5 / 8-13。不自动上报，不上报请求正文与播放流地址。窄测试覆盖脱敏、空文件/超限拒绝、保留 30 份、关于页无日志提示与上传成功、后台列表、正文与确认删除。提交主题：`feat: 支持本机诊断日志手动上报与后台查看`。
...
### T25 服务端流超时修复（2026-09-15）

PlaybackController 改同步分块转发并覆盖全部上游关闭路径；RequestLoggingInterceptor 保留初次请求计时。窄测试覆盖同步 MVC、Range、读写失败、截断和资源关闭；真实 Tomcat 对照默认异步超时及修复后的持续传输，完整应用启动健康 UP。不得以本地模拟流替代线上原视频验收；API 与旧 App 兼容行为不变，决策见 ADR-0039。
