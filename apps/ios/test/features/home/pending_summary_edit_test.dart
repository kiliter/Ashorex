...
/// 原型 1-8 未完成汇总的编辑态：默认两个批量动作作用于全部筛选结果，
/// 进入编辑态后只作用于勾选项，未勾选时必须禁用。
void main() {
  testWidgets('默认态两个批量动作作用于全部筛选结果', (tester) async {
    await _pump(tester, _backend());

    expect(find.text('未完成汇总'), findsOneWidget);
    expect(find.text('全部顺延今天'), findsOneWidget);
  ...
  testWidgets('进入编辑态后按钮换成计数文案，未勾选时禁用', (tester) async {
    ...
    expect(_button(tester, '顺延 0 项到今天').onPressed, isNull);
    expect(_button(tester, '删除 0 项').onPressed, isNull);
  });

  testWidgets('勾选后批量顺延只提交所选项', (tester) async {
    ...
    await tester.tap(find.text('顺延 1 项到今天'));
    await tester.pumpAndSettle();

    final body = backend.lastRequest('POST', '/api/v1/todos/batch-defer').json;
    expect(body['todoIds'], ['t-1']);
    expect(body['targetDate'], '2030-01-02');
  });

  testWidgets('批量顺延过滤下架课时，删除入口仍保留', (tester) async {
    final backend = _backend(courseAvailable: false)
      ..on('POST', '/api/v1/todos/batch-defer');
    await _pump(tester, backend);
    expect(find.text('批量清理'), findsOneWidget);
    await tester.tap(find.text('可用项顺延今天'));
    await tester.pumpAndSettle();
    final body = backend.lastRequest('POST', '/api/v1/todos/batch-defer').json;
    expect(body['todoIds'], ['t-2']);
    expect(body['targetDate'], '2030-01-02');
  });
