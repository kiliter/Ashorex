...
/// 我的页：催办策略与免打扰只读；个人 Bark 在独立面板允许当前用户配置。
...
void main() {
  ...
  testWidgets('催办阈值与免打扰只读，个人 Bark 提供独立配置入口', (tester) async {
    await _pump(tester, _backend());
    ...
    // 说明位于新增个人推送卡下方，先滚动到用户可见区域。
    await tester.scrollUntilVisible(
      find.text('催办阈值和免打扰由服务端维护；个人 Bark 可在上方自行配置。'),
      150,
    );
    expect(find.text('催办阈值和免打扰由服务端维护；个人 Bark 可在上方自行配置。'), findsOneWidget);
    // 主页面策略区仍只读，个人开关在独立面板内。
    expect(find.byType(Switch), findsNothing);
  });

  testWidgets('个人 Bark 读取不回显密钥，开关保存只提交当前账号配置', (tester) async {
    ...
    await tester.ensureVisible(find.text('我的 Bark 推送'));
    await tester.tap(find.text('我的 Bark 推送'));
    await tester.pumpAndSettle();
    expect(
      tester.widget<TextField>(find.byType(TextField).last).controller!.text,
      isEmpty,
    );
    expect(find.text('已配置，留空保持原值'), findsOneWidget);
    await tester.tap(find.byType(Switch));
    await tester.pumpAndSettle();
    await tester.tap(find.text('保存'));
    await tester.pumpAndSettle();
    expect(backend.lastRequest('PUT', '/api/v1/me/bark').json, {
      'baseUrl': 'https://api.day.app',
      'deviceKey': '',
      'enabled': true,
    });
    expect(find.text('启用个人 Bark'), findsNothing);
    expect(tester.takeException(), isNull);
  });
