...
<template>
  <div class="upgrade-page">
    ...
    <section class="upgrade-card version-card" aria-label="服务端版本">
...
/* 页面占满后台可用高度；仅内容区在小窗口或长文案时滚动，避免撑高整个后台。 */
.upgrade-page { width: 100%; max-width: 1200px; height: 100%; min-height: 0; display: flex; flex-direction: column; gap: 12px; }
.upgrade-page > header, .upgrade-page > p { flex-shrink: 0; }
.upgrade-page > p, .upgrade-page header p { margin: 0; }
.upgrade-page h1 { margin: 0 0 6px; font-size: 22px; }
.upgrade-content { flex: 1; min-height: 0; overflow: auto; display: grid; grid-template-columns: minmax(0, 1fr) minmax(0, 1fr); gap: 16px; align-content: start; overscroll-behavior: contain; }
.upgrade-content > .upgrade-card { margin: 0; min-width: 0; padding: 20px; }
.version-card { grid-row: span 2; }
.upgrade-card h2 { margin: 0 0 12px; }
.upgrade-card p { margin: 10px 0; overflow-wrap: anywhere; }
.actions { margin: 16px 0; }
.settings-grid { gap: 12px; }
.settings-grid input { width: 100%; min-width: 0; }
.notes { max-height: 40dvh; overflow: auto; }
/* 确认框也必须适配矮窗口，保留确认与取消按钮的可达性。 */
.confirm-box { max-height: calc(100dvh - 40px); overflow: auto; }
@media (max-width: 1000px) {
  .upgrade-content { grid-template-columns: minmax(0, 1fr); }
  .version-card { grid-row: auto; }
}
</style>
